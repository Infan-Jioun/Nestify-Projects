...
export default function ShopPage() {
  ...
  const [shopProperties, setShopProperties] = useState<PropertyType[]>([])
  ...
  useEffect(() => {
    const shops = properties.filter(property =>
      property.category?.name?.toLowerCase() === 'shop'
    )
    setShopProperties(shops)
  }, [properties])
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="py-8">
        <div className="container mx-auto px-4">
          ...
          {/* Empty State */}
          {!loading && !error && shopProperties.length === 0 && (
            <div className="text-center py-12">
              <div className="bg-white rounded-xl shadow-sm p-8 max-w-md mx-auto">
                <svg className="w-16 h-16 text-gray-400 mx-auto mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
                </svg>
                <h2 className="text-xl font-semibold text-gray-900 mb-2">No Shop Properties Found</h2>
                <p className="text-gray-600 mb-6">
                  {"We couldn't find any shop properties matching your criteria."}
                </p>
                <button
                  onClick={() => dispatch(fetchProperties())}
