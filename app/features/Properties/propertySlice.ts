...
export const updatePropertyStatus = createAsyncThunk<
  ...
  { rejectValue: string }
>("properties/updateStatus", async ({ propertyId, status }, { rejectWithValue, getState }) => {
  try {
    // ✅ আগে current property data fetch করো
    const propertyResponse = await axios.get<PropertyType>(`/api/properties/${propertyId}`);
    const existingProperty = propertyResponse.data;

    // ✅ সব required fields সহ status update করো
    const response = await axios.put<{ property: PropertyType }>(
      `/api/properties/${propertyId}`,
      {
        ...
        propertySize: existingProperty.propertySize,
        contactNumber: existingProperty.contactNumber,
        address: existingProperty.address,
        geoCountryLocation: existingProperty.geoCountryLocation,
        email: existingProperty.email,
        listingStatus: existingProperty.listingStatus,
        images: existingProperty.images,
        videos: existingProperty.videos,
        propertyFacilities: existingProperty.propertyFacilities,
        status: status, // ✅ শুধু status change হবে
      }
    );
    return response.data.property;
  } catch (err: unknown) {
    const error = err as AxiosError<{ message: string }>;
    console.error("updatePropertyStatus error:", error.response?.data);
    return rejectWithValue(
      error.response?.data?.message ||
