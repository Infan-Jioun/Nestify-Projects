...
export const fetchPropertyById = createAsyncThunk<PropertyType, string, { rejectValue: string }>(
    ...
    toggleFavorite: (state, action: PayloadAction<string>) => {
      ...
      if (property) {
        if (!("isFavorite" in property)) {
          (property as any).isFavorite = true;
        } else {
          (property as any).isFavorite = !(property as any).isFavorite;
        }
      }
    }
  },
