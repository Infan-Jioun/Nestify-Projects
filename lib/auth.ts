  ...
  callbacks: {
    async signIn({ user, account, profile }: SignInParams) {
      try {
        ...
        const img = user?.image || (profile as ExtendedProfile)?.picture || (profile as ExtendedProfile)?.avatar_url || null
        const base = {
          name: user?.name || profile?.name || 'User',
          email,
          image: img,
          provider: account.provider as 'google' | 'github',
          providerAccountId: account.providerAccountId ?? (profile as ExtendedProfile)?.id?.toString() ?? null,
          role: UserRole.USER,
        }
    ...
    async session({ session, token }) {
      try {
        if (session.user && token.id && token.name && token.email && token.role) {
